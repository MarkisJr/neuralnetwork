...
class Window extends JPanel
{
	...
	public void init()
	{
		JToggleButton pixel[][] = new JToggleButton[24][24];
		
		for (int y=0; y<24; y++)
		{
			for (int x=0; x<24; x++)
			{
				pixel[y][x] = new JToggleButton();
				pixel[y][x].setPreferredSize(new Dimension(10,10));
				...
				if (y % 2 == 0)
				{
					...
					switch (temp) 
					{
						case 0: pixel[y][x].setBackground(Color.WHITE);
						break;
						case 1: pixel[y][x].setBackground(Color.BLACK);
						break;
					}
				}
				else 
				{
					...
					switch (temp) 
					{
						case 0: pixel[y][x].setBackground(Color.BLACK);
						break;
						case 1: pixel[y][x].setBackground(Color.WHITE);
						break;
					}
				}
				add(pixel[y][x]);
...
public class Input 
{
	...
	public static void main(String[] args)
	{
		...
		w.setResizable(true);
		w.getContentPane().add(importclass);
		w.setVisible(true);
	}
